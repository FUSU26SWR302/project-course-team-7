...
-- =======================================================================
DROP TABLE IF EXISTS Payments;             -- Thêm mới (Xóa trước vì tham chiếu Appointments)
...
-- 5b. Blacklists — Quản lý danh sách đen bệnh nhân spam bùng hẹn
-- reason: Lưu lý do khóa (Ví dụ: "Bùng hẹn quá 3 lần trong vòng 30 ngày")
-- expires_at: Ngày hết hạn phạt để hệ thống tự động mở khóa. Nếu NULL = Khóa vĩnh viễn.
-- is_active: 1 = Đang bị chặn, 0 = Đã được gỡ phong tỏa (hết hạn hoặc Admin mở sớm)
...
CREATE TABLE Appointments (
    ...
    status           VARCHAR(50) DEFAULT 'Pending', -- 'Pending', 'Confirmed', 'Completed', 'Cancelled', 'NoShow'
    reason_cancelled NVARCHAR(255) NULL,
    cancelled_by     INT FOREIGN KEY REFERENCES Users(user_id) NULL
);
-- 17b. Payments — Quản lý tiền đặt cọc giữ chỗ của lịch hẹn
-- amount: Số tiền đặt cọc (có thể lấy cấu hình bằng 100% hoặc % cụ thể của examination_fee)
-- transaction_code: Mã giao dịch đối soát trả về từ cổng thanh toán (VNPay/MoMo)
-- status: 'Pending' (Chờ thanh toán), 'Paid' (Đã cọc thành công), 'Refunded' (Đã hoàn tiền khi hủy hợp lệ), 'Failed' (Lỗi)
